    ...
    <Stage>
      <svg viewBox="0 0 800 500" className="w-full h-full">
        ...
        <motion.text
          ...
          animate={{ opacity: 1 }}
        >
          {t('frame7_4.title', { defaultValue: 'The Badge Metaphor' })}
        </motion.text>
        ...
        >
          ...
          <text x={400} y={150} textAnchor="middle" fill={colors.agent} fontSize={14} fontWeight="bold" fontFamily="Space Grotesk, sans-serif">
            {t('frame7_4.securityDesk', { defaultValue: 'Security Desk' })}
          </text>
...
<text x={200} y={385} textAnchor="middle" fill={colors.textSecondary} fontSize={12}>{t('frame7_4.employeeWorking', { defaultValue: 'Employee working' })}</text>
        </motion.g>
        ...
        {phase >= 2 && (
          ...
          >
            ...
            <text x={575} y={310} textAnchor="middle" fill={colors.success} fontSize={13} fontWeight="bold" fontFamily="Space Grotesk, sans-serif">
              {t('frame7_4.zeroInterruption', { defaultValue: 'Zero interruption' })}
            </text>
            <text x={575} y={335} textAnchor="middle" fill={colors.textSecondary} fontSize={11} fontFamily="IBM Plex Sans, sans-serif">
              {t('frame7_4.metaphorText', { defaultValue: 'Employee never stops working' })}
            </text>
          </motion.g>
        )}
...
{t('frame7_4.autoRenew', { defaultValue: 'Your badge auto-renews every hour. You never think about it.' })}
            </text>
          </motion.g>
        )}
      </svg>
