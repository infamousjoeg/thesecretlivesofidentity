    ...
    <Stage>
      <svg viewBox="0 0 800 500" className="w-full h-full">
        ...
        <motion.text
          ...
          animate={{ opacity: 1 }}
        >
          {t('frame6_7.title', { defaultValue: 'Authentication ≠ Authorization' })}
        </motion.text>
        ...
        >
          ...
          <text x={225} y={130} textAnchor="middle" fill={colors.success} fontSize={16} fontWeight="bold" fontFamily="Space Grotesk, sans-serif">
            {t('frame6_7.authNLabel', { defaultValue: 'Authentication' })}
          </text>
          <text x={225} y={155} textAnchor="middle" fill={colors.textSecondary} fontSize={12} fontFamily="IBM Plex Sans, sans-serif">
            {t('frame6_7.authNQuestion', { defaultValue: '"Who are you?"' })}
          </text>
          <Workload label={t('frame6_7.serviceLabel', { defaultValue: 'service' })} position={{ x: 180, y: 220 }} attested={true} animate={!prefersReducedMotion} size={50} />
          <Badge spiffeId="spiffe://acme/svc" position={{ x: 270, y: 220 }} state="valid" size={40} animate={!prefersReducedMotion} />
          <text x={225} y={280} textAnchor="middle" fill={colors.success} fontSize={12} fontWeight="bold">
            {t('frame6_7.authNProvides', { defaultValue: '✓ SPIFFE provides this' })}
          </text>
        </motion.g>
        ...
        {phase >= 1 && (
          ...
          >
            ...
            <text x={575} y={220} textAnchor="middle" fill={colors.warning} fontSize={11} fontFamily="JetBrains Mono, monospace">
              {t('frame6_7.policyEngine', { defaultValue: 'Policy Engine' })}
            </text>
            <text x={575} y={245} textAnchor="middle" fill={colors.textMuted} fontSize={10}>
              {t('frame6_7.policyEngineTools', { defaultValue: 'OPA, Service Mesh, etc.' })}
            </text>
            <text x={575} y={280} textAnchor="middle" fill={colors.warning} fontSize={12} fontWeight="bold">
              {t('frame6_7.separateSystem', { defaultValue: '↳ Separate system' })}
            </text>
          </motion.g>
        )}
        ...
        {phase >= 2 && (
          ...
          >
            ...
            <text x={400} y={370} textAnchor="middle" fill={colors.textPrimary} fontSize={14} fontWeight="600" fontFamily="Space Grotesk, sans-serif">
              {t('frame6_7.spiffeProvides', { defaultValue: 'SPIFFE provides AUTHENTICATION' })}
            </text>
            <text x={400} y={395} textAnchor="middle" fill={colors.textSecondary} fontSize={13} fontFamily="IBM Plex Sans, sans-serif">
              {t('frame6_7.authNExplanation', { defaultValue: '(proving who you are), not AUTHORIZATION (what you can do)' })}
            </text>
          </motion.g>
        )}
        ...
        {phase >= 3 && (
          ...
          >
            <text x={400} y={450} textAnchor="middle" fill={colors.textMuted} fontSize={13} fontFamily="IBM Plex Sans, sans-serif">
              {t('frame6_7.conclusion', { defaultValue: 'Authorization is handled separately by your application or policies' })}
            </text>
          </motion.g>
        )}
      </svg>
