    ...
    <Stage>
      <svg viewBox="0 0 800 500" className="w-full h-full">
        ...
        <motion.text
          ...
          animate={{ opacity: 1 }}
        >
          {t('frame6_9.title', { defaultValue: 'Authorization Layers' })}
        </motion.text>
        ...
        >
          ...
          <text x={400} y={385} textAnchor="middle" fill="white" fontSize={16} fontWeight="bold" fontFamily="Space Grotesk, sans-serif">
            {t('frame6_9.spiffeLayer', { defaultValue: 'SPIFFE Identity Layer' })}
          </text>
        </motion.g>
        ...
        {phase >= 1 && (
          ...
          >
            ...
            <text x={175} y={285} textAnchor="middle" fill={colors.svid} fontSize={13} fontWeight="bold" fontFamily="Space Grotesk, sans-serif">
              {t('frame6_9.yourApp', { defaultValue: 'Your Application' })}
            </text>
            <text x={175} y={305} textAnchor="middle" fill={colors.textMuted} fontSize={10}>
              {t('frame6_9.customLogic', { defaultValue: 'Custom logic' })}
            </text>
          </motion.g>
        )}
        ...
        {phase >= 2 && (
          ...
          >
            ...
            <text x={355} y={285} textAnchor="middle" fill={colors.agent} fontSize={13} fontWeight="bold" fontFamily="Space Grotesk, sans-serif">
              {t('frame6_9.serviceMesh', { defaultValue: 'Service Mesh' })}
            </text>
            <text x={355} y={305} textAnchor="middle" fill={colors.textMuted} fontSize={10}>
              {t('frame6_9.serviceMeshTools', { defaultValue: 'Istio, Linkerd' })}
            </text>
          </motion.g>
        )}
        ...
        {phase >= 3 && (
          ...
          >
            ...
            <text x={535} y={285} textAnchor="middle" fill={colors.warning} fontSize={13} fontWeight="bold" fontFamily="Space Grotesk, sans-serif">
              {t('frame6_9.opaLabel', { defaultValue: 'OPA / Rego' })}
            </text>
            <text x={535} y={305} textAnchor="middle" fill={colors.textMuted} fontSize={10}>
              {t('frame6_9.opaSub', { defaultValue: 'Policy engine' })}
            </text>
          </motion.g>
        )}
        ...
        {phase >= 4 && (
          ...
          >
            ...
            <text x={695} y={285} textAnchor="middle" fill={colors.trustBundle} fontSize={13} fontWeight="bold" fontFamily="Space Grotesk, sans-serif">
              {t('frame6_9.rbacLabel', { defaultValue: 'RBAC' })}
            </text>
            <text x={695} y={305} textAnchor="middle" fill={colors.textMuted} fontSize={10}>
              {t('frame6_9.rbacSub', { defaultValue: 'K8s, Cloud IAM' })}
            </text>
          </motion.g>
        )}
        ...
        {phase >= 4 && (
          ...
          >
            <text x={400} y={450} textAnchor="middle" fill={colors.textPrimary} fontSize={14} fontWeight="600" fontFamily="Space Grotesk, sans-serif">
              {t('frame6_9.conclusion', { defaultValue: 'SPIFFE tells you WHO—authorization policies tell you WHAT' })}
            </text>
          </motion.g>
        )}
      </svg>
